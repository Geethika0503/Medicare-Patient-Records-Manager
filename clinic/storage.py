...
def load_patients(csv_path: Path) -> tuple[list[Patient], list[str]]:
    ...
    Rules:
    - A bad row must NEVER crash the program.
    ...
    with csv_path.open("r", newline="", encoding="utf-8") as f:
        ...
        for line_no, row in enumerate(reader, start=2):
            ...
            if patient_id in seen_ids:
                errors.append(
                    f"line {line_no}: duplicate patient_id '{patient_id}'"
                )
                continue
...
def write_clean_json(
    patients: list[Patient],
    out_path: Path
) -> None:
    """Save the validated records as human-readable JSON."""
